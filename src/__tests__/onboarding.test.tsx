...
describe("AmitSells onboarding app", () => {
  ...
  it("shows review answers and calls the mock API on submit", async () => {
    ...
    setField(/transformation or outcome/i, "They build a predictable sales system.");
    setField(/specific problem/i, "Revenue leakage.");
    setField(/included inside your offer/i, "Coaching, reviews, and implementation.");
    setField(/choose you/i, "Strong proof and sharper positioning.");
    setField(/not a good fit/i, "People who will not implement.");
    await user.click(screen.getByRole("button", { name: /save & continue/i }));

    fireEvent.change(await screen.findByLabelText(/ideal buyer/i), {
      target: { value: "High-ticket service founders." },
    });
    setField(/biggest struggles/i, "Low conversion.");
